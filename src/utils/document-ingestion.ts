...
import { DocumentChunk, DocumentMetadata } from '../types/personavault-types';
...
export class DocumentIngestionPipeline {
  ...
  async ingestAllDocuments(): Promise<IngestionStats> {
    const startTime = Date.now();
    console.log('Starting document ingestion...');

    try {
      // Ensure Qdrant index exists
  ...
  private async ensureQdrantIndex(): Promise<void> {
    try {
      ...
      if (!indexes.includes('personavault-documents')) {
        console.log('Creating Qdrant index...');
  ...
  private async extractTextFromFile(filePath: string): Promise<string> {
    ...
    switch (ext) {
      ...
      case '.doc':
        ...
        const docResult = await extractor.extract(fileBuffer);
        return docResult.getBody();
      case '.txt':
      case '.md':
  ...
  private async processSingleDocument(filePath: string): Promise<void> {
    ...
    try {
      ...
      // Note: skipExisting check removed due to API complexity
      // Files will be reprocessed if script is run multiple times
  ...
  private async chunkDocument(docText: string, metadata: DocumentMetadata, fileName: string): Promise<DocumentChunk[]> {
  ...
  private async embedAndStore(chunks: DocumentChunk[]): Promise<void> {
